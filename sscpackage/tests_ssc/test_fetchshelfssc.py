...
class Test_FetchShelfSSC(unittest.TestCase):
    ...
    def test_fetchshelfsscmock(self, ShelveMock):
        MockShelveVar = ShelveMock(
            spec_set=shelve.open
        )  # MagicMock Class Instance, takes the place for 'shelve'
        starterdict_mock = {}
        with patch.dict(
            starterdict_mock, {"TEST1": "VAlTEST1"}, clear=True
        ) as fetchpatchedshelf:
        ...
        self.assertEqual(
            "MSFT__url_income__DEFAULTID__DEFTSID", fetchstorenamefromFS1, "First"
        )
        self.assertTrue(ShelveMock.called)
        ShelveMock.assert_called_with(
            r"C:\SSC\SimpleStockChecker_REV1\sscpackage\storage\fetchfiledb"
        )
