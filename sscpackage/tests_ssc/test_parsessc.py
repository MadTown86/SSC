...
class MyTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(self.storpath, "w") as sfile:
            primedata_testlogfetch += (
                "MSFT__url_income__2510381479104__x1jU288DK5DRjWf, "
    ...
    def test_parsessc(
        ...
        type(MockFetchS()).fetchdbpull = unittest.mock.MagicMock(
            return_value=unittest.mock.MagicMock()
        )
        type(MockFetchL()).ssc_logfetch = unittest.mock.MagicMock(
            return_value=primedatatestlogfetchmock
        )
        type(MockParseSec()).parsesector = unittest.mock.MagicMock(
            return_value="Sector"
        )
        type(MockParseInd()).parseindustry = unittest.mock.MagicMock(
            return_value="Industry"
        )
        type(MockParseVal()).parseval = unittest.mock.MagicMock(return_value="ParseVal")
        type(MockParseInc()).parseincome = unittest.mock.MagicMock(
            return_value="ParseInc"
        )
        type(MockParseBal()).parsebalance = unittest.mock.MagicMock(
            return_value="ParseBal"
        )
