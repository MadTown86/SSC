...
class ParseIncomeSSC_Sub(parseincomessc.ParseIncome):
    ...
    def parseincome(self, uniquename: 'str', pi_rawdata: json) -> None:
        ...
        try:
            ...
            output_dict_inc = parsebalancessc_sub.incbal_reformat(uniquename,
                                                                  inner_wdict, keywordtransferbin)


            fetchstorename = uniquename
            FST_SSC = fetchshelfssc_mod.FetchShelfSSC(fetchstoreshelf=self.setpathssc_parsessc)
            FST_SSC.fetchstore(ticker=ticker, fetch_data=output_dict_inc, fetchstorename=fetchstorename)
            del FST_SSC
