...
def parsetool():
    ...
    try:
        with open("sectordata.json", 'r') as sd:
            parsetool.sector = dictpull(json.load(sd), "Sector")
            print(str(parsetool.sector))
            sd.close()
    ...
    try:
        with open("sectordata.json", 'r') as sd:
            parsetool.industry = dictpull(json.load(sd), "Industry")
            print(str(parsetool.sector))
            sd.close()
    ...
    with open("incomestatements.json", 'r') as isheets:
        ...
        for x in range(len(bsheets_keys)):
            bsheets_dict[bsheets_keys[x]] = parsetool.bsheets[x]

        isheets_dict = {}
        for x in range(len(isheets_keys)):
            isheets_dict[isheets_keys[x]] = parsetool.isheets[x]

        parsetool.idict = isheets_dict
        parsetool.bdict = bsheets_dict
        ...
        with safe_open_w(in_post) as f:
            idict_renum = {}
            for x in range(len(parsetool.idict.keys())):
                idict_renum[x] = [x for x in parsetool.idict.keys()][x]
        ...
        with safe_open_w(bal_post) as f:
            bdict_renum = {}
            for x in range(len(parsetool.bdict.keys())):
                bdict_renum[x] = [x for x in parsetool.bdict.keys()][x]
...
def grade_tool(bsheets_zip: "list", isheets_zip: "list", isheets_dict: "dict", bsheets_dict: "dict") -> "str":
    """
    8/14/2021
    Author: GD
    ...
    in_sheets = parsetool.isheets
    bal_sheets = parsetool.bsheets
    ...
    with safe_open_w(path_gs + pre_fix) as f:
        ...
        f.write("SECTOR DESIGNATOR:  " + str(parsetool.sector).upper() + (brn*2))
        f.write("INDUSTRY DESIGNATOR:  " + str(parsetool.industry).upper() + (brn*2))
        ...
        for x in l_rev:
            ...
            if x not in [y for y in idict.keys()]:
                ...
                grade_tool.tp += 6  # add 6 points to total for each loop
                flag = 0
                l_ishell = idict[x]
                ...
                if flag == 0:
                    if l_ishell[0] > l_ishell[1] and l_ishell[2]:
                        gv += 6
                        f.write("GRADE::: " + str(gv) + ' / ' + str(grade_tool.tp) + "\n")
                    ...
                    # If latest year is greater than prior 1 year and less than 2nd get 5 points
                        ...
                        gv += 5
                        f.write(str(gv) + ' / ' + str(grade_tool.tp)+ "\n")
        ...
        for x in l_1:
            ...
            if x not in [y for y in idict.keys()]:
                ...
                l_ishell = idict[x]
                grade_tool.tp += 2  # add 2 points to total
                """
        ...
        f.write("POINTS AWARDED: " + str(gv) + " / " + str(grade_tool.tp) + " = TOTAL POINTS\n")
        f.write("DECIMAL VALUE == " + str((gv / grade_tool.tp)))
        ...
        grade_tool.g = dec_gr
        grade_tool.erlist = erlist

        # I am creating this variable to house the A, B, C value until I pass it to the functions attribute
        tg = []

        # Overall Grade Scale
        if int(grade_tool.g) >= .94:
            tg.append("A")
        elif int(grade_tool.g) < .94 and grade_tool.g >= .90:
            tg.append("AB")
        elif int(grade_tool.g) < .90 and grade_tool.g>= .84:
            tg.append("B")
        elif int(grade_tool.g) < .84 and grade_tool.g>= 80:
            tg.append("BC")
        elif int(grade_tool.g) < .80 and grade_tool.g>= .74:
            tg.append("C")
        elif int(grade_tool.g) < .74 and grade_tool.g>= .70:
            tg.append("CD")
        elif int(grade_tool.g) < .70 and grade_tool.g>= 64:
            tg.append("D")
        elif int(grade_tool.g) < .64:
            tg.append("F")

        grade_tool.grade = tg[0]
